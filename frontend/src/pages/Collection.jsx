...
const Collection = () => {
  ...
  useEffect(() => {
    const fetchCollection = async () => {
      try {
        ...
        const response = await getCollection(slug, currentPage, filters);
  ...
  // Get products safely - must be before any conditional returns
  // Handle new API structure: products are in manualProducts or automaticProducts based on type
  const products = useMemo(() => {
    if (!collection) return [];
    if (collection.type === "manual" && collection.manualProducts) {
      return collection.manualProducts.products || [];
    }
    if (collection.type === "automatic" && collection.automaticProducts) {
      return collection.automaticProducts.products || [];
    }
    // Fallback for old structure or edge cases
  ...
  // Extract unique filter options from all products in collection (for display purposes)
  // Note: This is just for showing available options, actual filtering is done on backend
  ...
  return (
    <div className="w-full bg-white min-h-[calc(100vh-200px)]">
      <div className="mx-auto w-full max-w-480 px-6 pt-12 pb-16 sm:px-10 xl:px-16">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">
                {(() => {
                  // Get pagination from correct field based on collection type
                  const pagination =
                    (collection?.type === "manual" &&
                      collection?.manualProducts?.pagination) ||
                    (collection?.type === "automatic" &&
                      collection?.automaticProducts?.pagination) ||
                    collection?.pagination;

                  if (pagination) {
                    return `${pagination.totalProducts} ${pagination.totalProducts === 1 ? "Product" : "Products"}`;
                  }
                ...
                >
                  {products.map((product) => {
                    ...
                    const renderStars = (rating) => {
                      ...
                      for (
                        let i = fullStars + (hasHalfStar ? 1 : 0);
                        i < 5;
                        i++
                      ) {
                        stars.push(
